...
class _SamplingAndOcclusionBaseAlgo:
    def __init__(self, model, tokenizer, output_path, configs):
        ...
        self.pad_token = tokenizer.encoder[tokenizer.pad_token] if configs.base_model in ['roberta'] else tokenizer.vocab[tokenizer.pad_token]
    ...
    def get_ngram_mask_region(self, region, inp):
        # find the [PAD] token
        idx = 0
        while idx < len(inp) and inp[idx] != self.pad_token:
            idx += 1
        return [(max(region[0] - self.nb_range, 1), min(region[1] + self.nb_range, idx - 2))]
    
    def get_ngram_mask_region_pair(self, region, inp):
        i, j = region
        ...
        return [(max(i - self.nb_range, 1), min(i + self.nb_range, idx - 2)),
                (max(j - self.nb_range, 1), min(j + self.nb_range, idx - 2))]
...
class _SamplingAndOcclusionAlgo(_SamplingAndOcclusionBaseAlgo):
    ...
    def occlude_input_with_masks_and_run(self, inp_bak, inp_mask, segment_ids, x_regions, nb_regions, label,
                                         return_variable=False, additional_mask=[], have_target=False):
        ...
        for sample_i in range(self.sample_num + extra):
            ...
            for i in range(len(filled_inp)):
                ...
                if not (i in additional_mask and not x_region[0] <= i <= x_region[1]): # additional mask enabled
            ...
            for i in range(len(filled_inp)):
                ...
                if not x_region[0] <= i <= x_region[1] and i not in additional_mask: # additional mask enabled
        ...
        if have_target:
            logits_enb = torch.softmax(logits_enb, dim=-1)   # normalize to [0,1]
            logits_ex = torch.softmax(logits_ex, dim=-1)
    ...
    def occlude_pair_and_run(self, inp_bak, inp_mask, segment_ids, x_regions, nb_regions, label,
                                         return_variable=False, additional_mask=[], have_target=False):

        def in_regions(idx):
            for x_region in x_regions:
                if x_region[0] <= idx <= x_region[1]:
                    return True
            return False
        ...
        inp_lm = copy.copy(inp)
        for i in range(len(inp_lm)):
            for j, x_region in enumerate(x_regions):
                if nb_regions[j][0] <= i <= nb_regions[j][1] and not x_region[0] <= i <= x_region[1]:
                    inp_lm[i] = self.pad_token

        inp_th = torch.from_numpy(inp_lm).long().view(-1, 1)
        inp_length = torch.LongTensor([inp_length])
        fw_pos = torch.LongTensor([min(x_regions[0][1] + 1, len(inp))]) # TODO: consider x_regions[1]
        bw_pos = torch.LongTensor([max(x_regions[0][0] - 1, -1)])
        ...
        for sample_i in range(self.sample_num + extra):
            if sample_i == self.sample_num:
                ...
                if self.mask_outside_nb:
                    for i in range(len(filled_inp)):
                        for nb_region in nb_regions:
                            if 1 <= i < nb_region[0] or nb_region[1] < i <= inp_length - 2:
                                filled_inp[i] = self.pad_token

                len_bw = x_regions[0][0] - nb_regions[0][0]
                len_fw = nb_regions[0][1] - x_regions[0][1]
                if len_bw > 0:
                    filled_inp[nb_regions[0][0]:x_regions[0][0]] = bw_sample_seq[-len_bw:]
                if len_fw > 0:
                    filled_inp[x_regions[0][1] + 1:nb_regions[0][1] + 1] = fw_sample_seq[:len_fw]

            filled_inp_, mask_inp_ = [], []
            for i in range(len(filled_inp)):
                if not (i in additional_mask and not in_regions(i)):
                    filled_inp_.append(filled_inp[i])
                    mask_inp_.append(inp_mask[i])
                else:
                    filled_inp_.append(self.pad_token)
            ...
            for i in range(len(filled_inp)):
                if not in_regions(i) and i not in additional_mask: # additional mask enabled
                    filled_ex.append(filled_inp[i])
                    mask_ex.append(inp_mask[i])
                else:
                    if self.configs.keep_other_nw:
    ...
    def do_attribution(self, input_ids, input_mask, segment_ids, region, label=None, return_variable=False,
                       additional_mask=[], occlude_pair=False, have_target=False):
        ...
        if not occlude_pair:
            ...
            score = self.occlude_input_with_masks_and_run(inp_flatten, inp_mask_flatten, segment_ids, [region],
                                                      mask_regions, label, return_variable, additional_mask, have_target=have_target)
        else:
            regions = [(region[0], region[0]), (region[1], region[1])]
            mask_regions = self.get_ngram_mask_region_pair(region, inp_flatten)
            score = self.occlude_pair_and_run(inp_flatten, inp_mask_flatten, segment_ids, regions,
                                              mask_regions, label, return_variable, additional_mask, have_target=have_target)
        
        return score

    def do_attribution_pad_variant(self, input_ids, input_mask, segment_ids, region, label=None, return_variable=False,
                                   additional_mask=[], occlude_pair=False):
        """
