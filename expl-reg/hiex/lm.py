...
class AttnBiLSTM(nn.Module):
    def __init__(self, config, vocab, num_labels=2, n_layers=1, pad_token=0, device=0):
        super().__init__()
        self.hidden_size = config.lm_d_hidden
        self.embed_size = config.lm_d_embed
        self.n_vocab = len(vocab)
        self.input_size = self.n_vocab + 2
        self.gpu = 0
        self.device = device

        self.embedding = nn.Embedding(self.input_size, self.embed_size)
        self.lstm = nn.LSTM(self.embed_size, self.hidden_size, n_layers, bidirectional=True)
        self.out = nn.Linear(self.hidden_size, num_labels)

    def attn(self, lstm_out, final_hidden, attention_mask):
        # hidden = final_hidden.squeeze(0)
        hidden = final_hidden
        attn_weights = torch.bmm(lstm_out, hidden.unsqueeze(2)).squeeze(2) 
        attn_weights = attn_weights * attention_mask.float()
        soft_attn_weights = F.softmax(attn_weights, 1)
        new_hidden = torch.bmm(lstm_out.transpose(1, 2), soft_attn_weights.unsqueeze(2)).squeeze(2)
        return new_hidden
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        batch_size = input_ids.size(0)          # [B,T]
        embedded = self.embedding(input_ids)    # [B,T,E]
        embedded = embedded.transpose(0, 1)     # [T,B,E]

        h_0 = torch.zeros(2, batch_size, self.hidden_size).to(self.device)
        c_0 = torch.zeros(2, batch_size, self.hidden_size).to(self.device)
		
        output, (final_hidden, final_cell) = self.lstm(embedded, (h_0, c_0))
        output = output.transpose(0, 1) # [B,T,2H]
        
        fb_out = output[:, : , :self.hidden_size] + output[:, : , self.hidden_size:]
        fb_hidden = final_hidden[0, :, :] + final_hidden[1, :, :]

        attn_output = self.attn(fb_out, fb_hidden, attention_mask)
